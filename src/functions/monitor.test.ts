import { OrchestrationContext } from "durable-functions/types/orchestration";
...
describe("monitor orchestrator", () => {
  it("polls 2 times until status is DONE", () => {
    ...
    result = generator.next("PENDING"); //injects Pending to the last yield expression
    expect(result.done).toBe(false);
    ...
    result = generator.next("DONE"); //Injects DONE to the last yield expression
